...
public abstract sealed class Task permits EpicTask, RegularTask, SubTask {
    ...
    private static int getValidatedId(int id) {
        if (id < 0) {
            throw new IllegalArgumentException("The Task ID must be greater than 0. Provided ID: " + id);
        }
        return id;
    }
    ...
    private static String getValidatedTitle(String title) {
        ...
        if(title.length() < MIN_TITLE_LENGTH) {
            throw new IllegalArgumentException("Title length should be greater than " + MIN_TITLE_LENGTH +
                    ". Your title is: \""  + title + "\".");
        }
        return title;
    }
    ...
    private static String getValidatedDescription(String description) {
        ...
        if(description.length() < MIN_DESCRIPTION_LENGTH) {
            throw new IllegalArgumentException("Description length should be greater than " + MIN_DESCRIPTION_LENGTH +
